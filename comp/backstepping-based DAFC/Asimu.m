...
for i=4:7
    ...
    [Eta{i}(1,:), Etad{i}(1,:), Etadd{i}(1,:), V{i}(1,:), C__{i}(1,:), D__{i}(1,:), M__{i}(1,:), F{i}(1,:)] = ...
...
for t=tr(1):dt:tr(end-1)
    ...
    for i=1:3
        ...
        Etad{i}(loop+1,:) = [desPosd{i}(loop+1,:), desYawd{1}(1)];
        Etadd{i}(loop+1,:) = [desPosdd{i}(loop+1,:), desYawdd{1}(loop+1)];
    end        

    for i=4:7
        tao1 = ctrPos(i, w, D, gamma1, Eta, Etad, Etadd, F, loop);
        tao2 = ctrYaw(i, D, gamma2, Eta, Etad, Etadd, F, loop);        
        Tao{i}(loop,:) = [tao1, tao2];        
        
        [Eta{i}(loop+1,:), Etad{i}(loop+1,:), Etadd{i}(loop+1,:), V{i}(loop+1,:), C__{i}(loop+1,:), D__{i}(loop+1,:), M__{i}(loop+1,:), F{i}(loop+1,:)] = ...
